...
function initializeSnakeLadder() {
  canvas = document.querySelector("canvas");
  wrapper = document.querySelector(".wrapper");
  resetGameBtn = document.querySelector("#reset");
  diceDisplay = document.querySelector("#diceThrow");
  playerDisplay = document.querySelector(".playerName");
...
const boustrophedonWalk = (cols, rows) => {
  ...
  for (let row = 0; row < rows; row++) {
    let t = Array.apply(null, Array(cols)).map((x, col) => {
      return {
        id: col + row * cols,
        y: height - gridSize - row * gridSize,
        x: col * gridSize,
      };
    });
    t = row % 2 ? t.reverse() : t;
  ...
  if (activePlayer.current >= activePlayer.target) {
    ...
    for (let i = 0; i < obstacles.length; i++) {
      if (obstacles[i].start === activePlayer.target) {
        ...
        slide(
          activePlayer,
          walkSequence[endSquare - 1].x,
          walkSequence[endSquare - 1].y,
          slideSpeed
        );
        break;
      }
...
const rollDice = (evt) => {
  ...
  message.innerHTML =
    activePlayer === player1 ? "Rolling..." : "Auto rolling...";
...
const drawObstacles = () => {
  ...
  for (let i = 0; i < obstacles.length; i++) {
    ...
    ctx.beginPath();
    ctx.moveTo(
      walkSequence[ob.start - 1].x + gridSize * 0.5,
      walkSequence[ob.start - 1].y + gridSize * 0.5
    );
    ctx.lineTo(
      walkSequence[ob.end - 1].x + gridSize * 0.5,
      walkSequence[ob.end - 1].y + gridSize * 0.5
    );
    ctx.stroke();
    ctx.closePath();
  }
